...
class User:
    ...
    @classmethod
    def create(cls, data):
        query = "INSERT INTO `red_finale`.`user` (`firstname`, `lastname`, `email`, `password`) VALUES (%(firstname)s, %(lastname)s, %(email)s, %(password)s);"
    ...
    def verify_email(cls,data):
        ...
        result = connectToMySQL("red_finale").query_db(query,data)
        if len(result)  != 0:
    ...
    @classmethod
    def update(cls, data):
        query = "UPDATE `red_finale`.`user` SET `firstname` = %(firstname)s, `lastname`=%(lastname)s, `email`= %(email)s, `password`= %(password)s WHERE (`id` = %(id)s);"
        return connectToMySQL('red_finale').query_db(query)
    ...
    @classmethod
    def delete(cls, data):
        query = "DELETE FROM `red_finale`.`user` WHERE (`id` = %(id)s);"
        return connectToMySQL('red_finale').query_db(query)
